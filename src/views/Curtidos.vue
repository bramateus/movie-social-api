        ...
        <div class="col-lg-3 col-sm-6 col-6" v-for="(item, index) in itens" :key="index">
          <div class="card" @click.prevent="show(index)">
            ...
            <div class="container" style="padding: 15px 0 0 0">
              ...
              <!-- <a @click.prevent="show" href="#">Ver Sinopse</a> -->
...
export default {
  ...
  data() {
    return {
      itens: [],
      data: this.$store.state.filmes_curtidos,
      index: 0,
    }
  },
  created() {
     console.log("CRETA QUI");
    //  document.body.style.background = 'linear-gradient( to bottom, rgba(255,28,28,0.9), rgba(255,86,86,0.9) )';
    if (localStorage.getItem("social-movie")) {
      var localStorageDataCurtidos = localStorage.getItem("social-movie");
      this.itens = JSON.parse(localStorageDataCurtidos)[0].curtidos.slice().reverse();
